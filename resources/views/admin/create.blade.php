    ...
    <div class="max-w-lg mx-auto mt-10 p-6 bg-white rounded-2xl shadow-md border border-gray-100">
        ...
        <form action="{{ route('admin.stands.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Kategori Utama Stan</label>
                    <select name="category" required class="w-full p-3 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:border-orange-500">
                        <option value="makanan">🍛 Stan Makanan Berat (Nasi, Mie, Ayam, Bakso, dll)</option>
                        <option value="cemilan">🍿 Stan Cemilan & Snack (Gorengan, Kue, Batagor, Takoyaki)</option>
                        <option value="minuman">🍹 Stan Aneka Minuman (Kopi, Jus, Es Teh, Boba, Susu)</option>
                    </select>
                </div>
